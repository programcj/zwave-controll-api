...
//curl_easy_setopt(curl,CURLOPT_TIMEOUT,1);
	...
	if (res == CURLE_OK)
		res = curl_easy_getinfo_response_code(curl, &retcode);
	...
	return retcode == 200 ? 0 : (int) retcode;
}
...
int http_post(const char *url, const char *args) {
	//char *url = "http://120.24.18.225:8080/hank_cloud_platform_api/devices/regedit";
	//char *args = "sn=HK1000021706090071NG&devtype=3&ver=1#1";
	CURLcode res;
	CURL *curl;
	struct HttpData data;
	//struct curl_slist *headers = NULL;
	...
	log_d("res=%d, http code=%d", res, retcode);
	if (res == CURLE_OK && retcode == 200) {
		return 0;
	}
	remove(path);
