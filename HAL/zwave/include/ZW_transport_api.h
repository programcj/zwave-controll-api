...
enum ZW_SENDDATA_EX_RETURN_CODES {
	ZW_TX_FAILED = 0, ZW_TX_IN_PROGRESS = 1, ZW_NO_SECURITY = 2
};
 ...
 **--------------------------------------------------------------------------*/
void
ZW_LockRoute(BOOL bLockRoute); /* IN TRUE lock LastWorkingRoute entry purging */
...
void
ZW_SendConst(BYTE bStart, /*IN TRUE start sending RF test signal, FALSE disable RF test signal*/
...
/*
 extern BYTE  ZW_SendData_FID(
 BYTE  destNodeID,           //IN  Destination node ID (0xFF == broadcast)
 BYTE *pData,                //IN  Data buffer pointer
 BYTE  dataLength,           //IN  Data buffer length
 BYTE  txOptions,            //IN  Transmit option flags
 BYTE *pFunId,
 VOID_CALLBACKFUNC(completedFunc)(BYTE funID,BYTE, TX_STATUS_TYPE*)); //IN  Transmit completed call back function
 */
#else
